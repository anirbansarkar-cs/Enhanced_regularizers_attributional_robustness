...
# Global constants
with open('eval_IG-SUM-NORM-config.json') as config_file:
...
m = config["m"]                                              #50
lamb = config["lambda"]                                      #1.0
approx_factor = config["approx_factor"]                      #5
...
model = Model('train', m, lamb, approx_factor)  ##(50,1.0,5)
...
if __name__ == "__main__":
  ...
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      ...
      x_batch = flower.eval_data.xs[bstart:bend, :]
      y_batch = flower.eval_data.ys[bstart:bend]
